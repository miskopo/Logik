...
class SemiRandom(SolvingAlgorithm):
    ...
    def guess_pattern(self):
        if len(self.guessed) == 0:
            true_random = TrueRandom(colors=self.colors, attempts=self.attempts, pattern_size=self.pattern_size)
            self.guessed = true_random.guess_pattern()
